...
const NewDepositEvent = (depositContract) => {
    ...
    depositContract.events.NewDeposit({}, async (error, event) => {
        console.log("test4", depositDetails)
        try {
            console.log("test5", depositDetails)
            if (!error) {
            ...
            //llogger.log('error','NewDepositEvent retuened Error : %s', JSON.stringify(err))
...
exports.createNewDeposit = async (depositDetails) => {
    console.log("test2", depositDetails)
 

    try {
    const accountData = await db.select('*').from('accounts').join('whitelist_status_lookup', 'whitelist_status_lookup.whitelist_status_id', '=', 'accounts.whitelist_status_id').where({address:depositDetails.address}).first()
    console.log("test2", accountData)
      ...
      console.log('info','addToDeposit with : %s', newDepositDetails)
      const depositAdded = await db.from('account_balance').insert(newDepositDetails)
      console.log('info','addToDeposit with : %s', depositAdded)
      return depositAdded
    } catch (error) {
        
        console.error('ERROR OCCURRED IN MUTATION(addAccount): %s', new Error(error))       
    }
  }
